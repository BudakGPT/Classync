...
| 18 Sep 16:00 | Erik | Excel/CSV Roster Upload, OpenRouter LLM Ingestion, NPM Discord Verification Gate, and Per-Class Channels | Integrated spreadsheet upload on web dashboard with OpenRouter free model + heuristic fallback, gated Discord onboarding through #verifikasi, automated role & nickname assignment (`NPM - Nama`), and provisioned isolated per-class categories (`📁 KELAS A`, `📁 KELAS B`). |
